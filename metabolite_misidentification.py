...
datasets_reactome = {"Auwerx": [DEM_auwerx, background_auwerx, Reactome_human_pathways, all_reactome_human_bg, mat_auwerx, [i for i in range(0, 7, 1)], [i for i in range(0, 6, 1)]],
                     "Yamada": [DEM_yamada, background_yamada, Reactome_human_pathways, all_reactome_human_bg, mat_yamada, [i for i in range(0, 12, 1)], [i for i in range(0, 9, 1)]],
                     # "Stevens": [DEM_stevens, background_stevens, Reactome_human_pathways, all_reactome_human_bg],
                     "Brown": [DEM_brown, background_brown, Reactome_mouse_pathways, all_reactome_mouse_bg, mat_brown, [i for i in range(0, 12, 1)], [i for i in range(0, 10, 1)]],
                     "Zamboni (yfgM)": [DEM_yfgM, background_yfgM, Reactome_human_pathways, all_reactome_human_bg, mat_yfgM],
                     "Zamboni (dcuS)": [DEM_dcuS, background_dcuS, Reactome_human_pathways, all_reactome_human_bg, mat_dcuS]}
...
def misidentification_mass_plot(db="KEGG"):
    ...
    for d in d_sets.keys():
        print(d)
        if d.startswith("Zamboni"):
            for i in [i for i in range(0, 5, 1)]:
                print(i)
    ...
    res_df = pd.DataFrame(results_lists, columns=["Dataset", "Percentage misidentification", "n_p_less_0.1", "n_q_less_0.1", "p_std", "q_std"])
    res_df.to_csv("Metabolite_misidentification_by_mass_simulation_KEGG.csv")
